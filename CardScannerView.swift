...
struct CardScannerView: View {
    ...
    // Bulk Batch Submission Trackers
    @State private var customBatchName: String = "Grading Shipment A"
    ...
    // MARK: - Main Tab Switcher Router (Expanded to 3 Production Segments)
    var body: some View {
    ...
    // MARK: - NEW: Sub-View 4: Staging Batch Manifest Control Dashboard Panel
    private var bulkBatchManifestView: some View {
        NavigationView {
            Form {
                Section(header: Text("BATCH SHIPMENT DESIGNATION")) {
                    TextField("Enter Submission Name...", text: $customBatchName)
                        .font(.body)
                    HStack {
                        Text("Staged Card Count:")
                        Spacer()
                        Text("(portfolio.savedCards.count) Items Ready").bold().foregroundColor(.blue)
                    }
                    HStack {
                        Text("Estimated Batch Value:")
                        Spacer()
                        Text(String(format: "$%.2f", portfolio.totalPortfolioValue)).bold().foregroundColor(.green)
                    }
                }
                Section(header: Text("EXPORT UTILITIES")) {
                    Button(action: {
                        isBatchExporting = true
                        // Fires print compilation workflow triggers inside core background worker frames
                        ...
                        if isBatchExporting {
                            ProgressView("Compiling Invoice Sheet...").frame(maxWidth: .infinity)
                        } else {
                            Label("Compile & Share CSV Manifest", systemImage: "doc.text.below.ecg.fill")
                                .bold().frame(maxWidth: .infinity)
                        }
                    }
                    .disabled(portfolio.savedCards.isEmpty)
                }
                Section(header: Text("SHIPMENT PACKING MANIFEST CHECKLIST")) {
                    if portfolio.savedCards.isEmpty {
                        Text("Add scanned items into your collection to generate a bulk shipment checklist.").font(.caption).foregroundColor(.secondary)
                    } else {
                        ForEach(portfolio.savedCards) { card in
                            HStack {
                                ...
                                VStack(alignment: .leading) {
                                    ...
                                    Text("Pre-Grade Check: PSA (card.predictedGradePSA)").font(.caption2).foregroundColor(.secondary)
                                }
                                Spacer()
